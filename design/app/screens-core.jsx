...
function Dashboard({ ctx }) {
  ...
  const [view, setView] = useStateC("overview");
  ...
  return (
    <div style={{ animation: "ll-fade .3s" }}>
      ...
      <div style={{ padding: "0 20px 14px" }}>
        <Segmented options={[{ value: "overview", label: "Overview" }, { value: "calendar", label: "Calendar" }]} value={view} onChange={setView} />
      </div>

      {view === "calendar" ? (
        <div style={{ padding: "0 20px 20px" }}>
          <LoanCalendar loans={LOANS} ctx={ctx} title="Collection calendar" sub={`All loanees · ${LOANEES.length} people`} />
        </div>
      ) : (
      <div style={{ padding: "0 20px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <PillButton variant="primary" icon="calc" size="md" onClick={() => nav.setTab("calc")} style={{ flex: 1 }}>New calculation</PillButton>
          <PillButton variant="outline" icon="plus" size="md" onClick={() => nav.push("loaneeForm")} style={{ flex: 1 }}>Loanee</PillButton>
        </div>
        ...
        <Card pad={16}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
            ...
            <div style={{ display: "flex", gap: 12, fontSize: 11.5, fontWeight: 650 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 5, color: "var(--text-soft)" }}><span style={{ width: 9, height: 9, borderRadius: 3, background: "var(--bar-track)" }} />Expected</span>
