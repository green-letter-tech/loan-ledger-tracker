...
function LoaneeDetail({ ctx, params }) {
  ...
  const [view, setView] = React.useState("loans");
    ...
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", animation: "ll-fade .3s" }}>
      ...
      <div style={{ flex: 1, padding: "4px 20px 8px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ marginTop: 2 }}>
          <Segmented options={[{ value: "loans", label: "Loans" }, { value: "calendar", label: "Calendar" }]} value={view} onChange={setView} />
        </div>
        {view === "calendar" ? (
          <LoanCalendar loans={active} ctx={ctx} title="Repayment calendar" sub={ln.name} />
        ) : (
        <div>
